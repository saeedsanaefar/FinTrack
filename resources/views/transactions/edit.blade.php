    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('transactions.show', $transaction) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                    View Transaction
                </a>
                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('transactions.update', $transaction) }}" id="transactionForm">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative">
                                <span id="currency-symbol" class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500">{{ $transaction->account->currency === 'USD' ? '$' : $transaction->account->currency }}</span>
                                <x-text-input id="amount" class="block mt-1 w-full pl-8" type="number" name="amount" :value="old('amount', $transaction->amount)" step="0.01" min="0.01" required placeholder="0.00" />
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="description" class="block mt-1 w-full" type="text" name="description" :value="old('description', $transaction->description)" required placeholder="Enter transaction description" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="transaction_date" class="block mt-1 w-full" type="date" name="transaction_date" :value="old('transaction_date', $transaction->transaction_date ? $transaction->transaction_date->format('Y-m-d') : '')" max="{{ date('Y-m-d') }}" required />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="reference" class="block mt-1 w-full" type="text" name="reference" :value="old('reference', $transaction->reference)" placeholder="Check number, invoice number, etc." />
                        ...
                        <div id="recurring-options" style="display: {{ old('is_recurring', $transaction->is_recurring) ? 'block' : 'none' }};">
                            <div class="mb-4">
                                ...
                                <select id="recurring_frequency" name="recurring_frequency" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    <option value="yearly" {{ old('recurring_frequency', $transaction->recurring_frequency) == 'yearly' ? 'selected' : '' }}>Yearly</option>
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <x-text-input id="recurring_end_date" class="block mt-1 w-full" type="date" name="recurring_end_date" :value="old('recurring_end_date', $transaction->recurring_end_date ? $transaction->recurring_end_date->format('Y-m-d') : '')" min="{{ date('Y-m-d', strtotime('+1 day')) }}" />
                        ...
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('transactions.show', $transaction) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-3">
                                Cancel
                            </a>
                            <x-primary-button>
